...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    """
    vPoints = np.array(nPointsX,nPointsY,2) # numpy array, [nPointsX*nPointsY, 2]
    # todo
    h, w = img.size()
    h = h - 2*border
    w = w - 2*border
    gridY = math.ceil(h/nPointsY)
    gridX = math.ceil(w/nPointsX)
    Y = np.arange(border+1,h-border,gridY)
    X = np.arange(border+1,w-border,gridX)
    [y,x] = np.meshgrid(Y,X)
    c = np.concatenate((y.T,x.T),axis=1)
    vPoints = np.reshape(c,([],2))
...
def descriptors_hog(img, vPoints, cellWidth, cellHeight):
    ...
    descriptors = np.zeros[0,nBins*4*4]  # list of descriptors for the current image, each entry is one 128-d vector for a grid point
    for i in range(len(vPoints)):
        vPoint = vPoints[i,:]
        xIds = [np.arange(vPoint[1]-2*w,vPoint(1)-1,1),np.arange(vPoint[1]+1,vPoint(1)+2*w,1)]
        yIds = [np.arange(vPoint[2]-2*h,vPoint(2)-1,1),np.arange(vPoint[2]+1,vPoint(2)+2*h,1)]
        xs = np.arange(vPoint[1]-2*w,vPoint(1)+2*w,1)
        ys = np.arange(vPoint[2]-2*h,vPoint(2)+2*h,1)
        cells = img[xIds,yIds]
        descriptor = []
        [angles,angles2] = np.arctan2(grad_x(xs,ys),grad_y(xs,ys))
        magnitudes = np.sqrt(grad_x(xs,ys)**2 + grad_y(xs,ys)**2)
        for j in range(0,16,4):
            for k in range(0,16,4):
                [n,_] = np.histogram(angles[j:j+3,k:k+3],8)
                [m,_] = np.histogram(magnitudes(j:j+3,k:k+3),8)
                descriptor = [descriptor n @ m]



        descriptors = [descriptors,descriptor] # [nPointsX*nPointsY, 128], descriptor for the current image (100 grid points)
    return descriptors
